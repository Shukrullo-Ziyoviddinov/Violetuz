...
const byScore = (a, b) => b.finalScore - a.finalScore || a.movieId.localeCompare(b.movieId);
...
const mixSlots = (candidates) => {
  const pattern = spreadPattern(homeFeedWeights.slotMix || {});
  ...
  const pull = (key) => {
    ...
    while (cursors[key] < queue.length) {
      ...
      cursors[key] += 1;
      if (used.has(row.movieId)) continue;
      used.add(row.movieId);
      return row;
    }
    return null;
  };
