...
import * as shortsLikesApi from '../../api/shortsLikesApi';
...
const ShortsVideos = ({ initialShorts = null, startIndex = null, onCloseFromHome = null, variant = 'movie' }) => {
  const { contentLang } = useContentLanguage();
  const { toggleWishlist, isInWishlist } = useWishlist();
  const baseList = initialShorts || shortsVideos;
  const isMusicShorts = variant === 'music';
  const [shortsList, setShortsList] = useState(() => baseList);
  ...
  const [shortsLikeCount, setShortsLikeCount] = useState(0);
  ...
  useEffect(() => {
    if (isMusicShorts || !modalOpen || activeIndex < 11 || hasLoadedMore.current) return;
  ...
  const desktopNavigate = useCallback((direction) => {
    if (slideState) return; // animatsiya ketayotsa blok
  ...
  }, [activeIndex, slideState, shortsList.length]);
  ...
  useEffect(() => {
    if (modalOpen && shortsList[activeIndex]) {
      setShortsLikeCount(shortsLikesApi.getLikeCount(shortsList[activeIndex].id));
    }
  }, [modalOpen, activeIndex]);

  const handleShortsLikeClick = useCallback((e) => {
    e.stopPropagation();
    const shortsId = shortsList[activeIndex]?.id;
    if (!shortsId) return;
    const nextCount = shortsLikesApi.toggleShortsLike(shortsId);
    setShortsLikeCount(nextCount);
  }, [activeIndex]);
  ...
  }, [activeIndex, toggleWishlist, isMusicShorts]);
  ...
  const modalContent = modalOpen && (
    <div className="shorts-modal-overlay" onClick={handleOverlayClick}>
      <div className="shorts-modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              >
                ...
                <div className={`shorts-modal-info shorts-modal-info-overlay shorts-modal-info-mobile ${descriptionExpanded ? 'shorts-modal-info-expanded' : ''}`}>
                  <div className="shorts-modal-info-content">
                    ...
                    <div className="shorts-modal-caption-music-wrap">
                      ...
                      {!isMusicShorts && shortsList[activeIndex]?.musics && (
                        <button
                          type="button"
                ...
                <div className="shorts-modal-actions-sidebar shorts-modal-actions-mobile">
                  ...
                  >
                    ...
                    {shortsLikeCount > 0 && <span className="shorts-modal-action-count">{formatLikeCount(shortsLikeCount)}</span>}
                  </button>
                  <button
                    type="button"
                  ...
                  <button
                    type="button"
                    className={`shorts-modal-action-btn shorts-modal-save-btn ${isMusicShorts ? (isInWishlist(shortsList[activeIndex]?.musicId, getMusicWishlistType(shortsList[activeIndex])) ? 'active' : '') : (isInWishlist(shortsList[activeIndex]?.movieId, 'movie') ? 'active' : '')}`}
                    onClick={handleShortsSaveClick}
                    aria-label="Saqlash"
                  >
                    <svg viewBox="0 0 24 24" fill={isMusicShorts ? (isInWishlist(shortsList[activeIndex]?.musicId, getMusicWishlistType(shortsList[activeIndex])) ? 'currentColor' : 'none') : (isInWishlist(shortsList[activeIndex]?.movieId, 'movie') ? 'currentColor' : 'none')} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {!isMusicShorts && musicModalOpen && shortsList[activeIndex]?.musics && createPortal(
