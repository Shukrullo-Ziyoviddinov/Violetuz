...
import './WatchSettingsModal.css';

const MOBILE_MQ = '(max-width: 768px)';

const WatchSettingsModal = ({
  isOpen,
  onClose,
  anchorRef,
  playbackSpeed,
  ...
  const [view, setView] = useState('main'); // main | language | speed
      ...
      >
        ...
        <div className="watch-settings-modal-body">
          ...
          >
            {view === 'main' && (
              <div className="watch-settings-modal-list">
                {hasLangSwitch && (
                  ...
                  >
                    <span>{t('player.videoLanguage')}</span>
                    <span className="watch-settings-modal-row-value">
                      {watchVideoLang?.toUpperCase()}
            ...
            {view === 'language' && (
              <div className="watch-settings-modal-list watch-modal-lang-row" role="group" aria-label={t('player.videoLanguage')}>
                {['uz', 'ru'].map((lang) => (
                  <button
                    key={lang}
                    type="button"
                    className={`watch-settings-modal-option watch-modal-lang-btn ${watchVideoLang === lang ? 'active' : ''}`}
                    onClick={() => {
                      onLangChange(lang);
                      goBack();
                    }}
                  >
                    {lang.toUpperCase()}
                    {watchVideoLang === lang && <span className="watch-settings-modal-check">✓</span>}
                  </button>
                ))}
              </div>
            )}
            ...
            {view === 'speed' && (
              <div className="watch-settings-modal-list watch-modal-speed-menu">
                {speedOptions.map((speed) => (
                  <button
                    key={speed}
                    type="button"
                    className={`watch-settings-modal-option watch-modal-speed-option ${playbackSpeed === speed ? 'active' : ''}`}
                    onClick={() => {
                      onSpeedChange(speed);
                      goBack();
                    }}
...
export default WatchSettingsModal;
