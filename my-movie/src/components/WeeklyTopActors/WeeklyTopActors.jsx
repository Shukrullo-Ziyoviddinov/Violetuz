...
import { useWeeklyTopActors } from '../../hooks/useWeeklyTopActors';
import { topRankSrc } from '../../utils/topRankPreview';
import '../TopActors/TopActors.css';

/**
 * Haftaning Top-10 aktyorlari (guest ham ko‘radi).
 * Data: /api/recommended-actors/weekly-top (rolling 7 days).
 */
const WeeklyTopActors = () => {
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();
  const lang = i18n.language === 'ru' ? 'ru' : 'uz';
  const { allActors } = useActorsApi();
  const { items, loading } = useWeeklyTopActors(10);
      ...
      <div className="top-actors-container">
        <div className="top-actors-header">
          <h2 className="top-actors-title">
            {t('movies.haftaningTopAktyorlari', 'Haftaning top 10 aktyorlari')}
          </h2>
        </div>
              ...
              : displayActors.map((actor) => (
                  ...
                  >
                    <div className="top-actors-img-wrap">
                      ...
                      />
                      <img
                        className="top-actors-rank"
                        src={encodeURI(topRankSrc(actor.rank))}
                        alt=""
                        aria-hidden="true"
                      />
                    </div>
...
export default WeeklyTopActors;
